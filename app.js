/*
A page that gives
- a description of one paragraph of a few progamming languages
  of your choice
- gives the user the option to show/hide a portion of the paragraph
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  languages.forEach(l => {
    l['shortDesc'] = l['description'].slice(0, 120) + "...";
    $languagesDiv.append(template(l));
  })

  $('button').click(e => {
    let divId = e.target.dataset.id;
    $shortDesc = $('#' + divId + " p.less");
    $wholeDesc = $('#' + divId + " p.whole");
    if (e.target.textContent === "Show More") {
      e.target.textContent = "Show Less"
      $wholeDesc.removeClass('hidden');
      $shortDesc.addClass('hidden');
    } else {
      e.target.textContent = "Show More"
      $wholeDesc.addClass('hidden');
